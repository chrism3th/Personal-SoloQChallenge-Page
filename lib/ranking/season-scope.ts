...
async function getLadderExtras(
  ...
  for (const [profileId, list] of byProfile) {
    ...
    for (const m of list) {
      championGames.set(m.championName, (championGames.get(m.championName) ?? 0) + 1);
      if (m.role) roleGames.set(m.role, (roleGames.get(m.role) ?? 0) + 1);
    }
    const topChampions = Array.from(championGames.entries())
    ...
    result.set(profileId, { recentForm, topChampions, preferredRole, lastGameLpDelta });
  }
...
export async function getLadder(
  ...
  const { data: liveRows } = await supabase
    .from("live_status")
    .select("profile_id, is_live");
  const liveSet = new Set(
    (liveRows ?? []).filter((row) => row.is_live).map((row) => row.profile_id)
  );
...
export async function getChampionStats(
  ...
  let query = supabase
    .from("match_participants")
    .select(
      "champion_id, champion_name, win, kills, deaths, assists, matches!inner(game_creation, queue_id)"
    )
  ...
  const byChampion = new Map<number, ChampionStat>();
  for (const row of rows) {
    const existing = byChampion.get(row.champion_id) ?? {
    ...
    const totalKills = existing.avgKills * existing.games + row.kills;
    const totalDeaths = existing.avgDeaths * existing.games + row.deaths;
    const totalAssists = existing.avgAssists * existing.games + row.assists;
    existing.games += 1;
    existing.wins += row.win ? 1 : 0;
    existing.losses += row.win ? 0 : 1;
    existing.avgKills = totalKills / existing.games;
    existing.avgDeaths = totalDeaths / existing.games;
    existing.avgAssists = totalAssists / existing.games;
    byChampion.set(row.champion_id, existing);
  }

  return Array.from(byChampion.values()).sort((a, b) => b.games - a.games);
}
