...
export function PodiumHero({ entries }: { entries: LadderEntry[] }) {
      ...
      {PODIUM_VISUAL_ORDER.map((idx) => {
        ...
        return (
          <Link
            ...
            href={`/jugador/${entry.profile.slug}`}
            className="focus-ring"
          >
            <Panel
              glowColor={entry.snapshot ? tierGlowVar(entry.snapshot.tier) : undefined}
              className="flex w-48 flex-col items-center gap-2 px-4 py-6"
            >
              <span className="font-display text-5xl text-accent">{place}</span>
              <span className="text-center font-body text-lg font-semibold">
                {entry.profile.display_name ?? entry.profile.riot_game_name}
              </span>
              {entry.snapshot ? (
                <TierBadge
